...
public class AdminReservationController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        req.getRequestDispatcher("/WEB-INF/views/admin/reservations_manage.jsp").forward(req, resp);
    }
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if ("/admin/reservation/cancel".equals(path)) {
            int reservationId = Integer.parseInt(req.getParameter("reservationId"));
            reservationDAO.cancelByStaffAdmin(reservationId);
            resp.sendRedirect(req.getContextPath() + "/admin/reservations?done=1");
            return;
        }

        if ("/admin/reservation/update".equals(path)) {
            int reservationId = Integer.parseInt(req.getParameter("reservationId"));
            ...
            resp.sendRedirect(req.getContextPath() + "/admin/reservations?updated=" + (ok ? "1" : "0"));
        }
    }
}
